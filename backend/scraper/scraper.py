...
class NarutoWikiScraper:
    ...
    def parse_character_list(self, page_content: str) -> list[str]:
        ...
        return [f'{self.wiki_url}{a["href"]}' for a in soup if "href" in a.attrs]

    def extract_character_data(self, character_page: str) -> dict:
            ...
            for element in parsed_content:
                if (text := element.text.strip()) and not (
                    ...
                    if element.name == "h2":
                        ...
                        if (
                            clean == "in_other_media"
                        ):  # Stop parsing at irrelevant sections
                            break
                        curr = [clean]
    ...
    async def fetch_character_details(
        ...
        character_data = self.extract_character_data(character_page)
    ...
    async def fetch_characters_by_letter(
        ...
        character_urls = self.parse_character_list(category_page)
